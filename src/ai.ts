...
/** Extra system instruction for the chosen analysis tone. */
export function toneInstruction(tone: string): string {
  switch (tone) {
    case "保守":
      return "語氣請保守謹慎，多強調風險、下檔與不確定性。";
    case "積極":
      return "語氣可較積極，明確點出潛在機會與題材，但仍須同時提示風險。";
    default:
      return "語氣請中性客觀，平衡呈現偏多與偏空兩面。";
  }
}
...
export function buildAnalysisPrompt(detail: StockDetail, rt?: RealtimeQuote): string {
  ...
  const changePct = live && s.ref_close ? ((rt!.last_price - s.ref_close) / s.ref_close) * 100 : s.change_pct;
